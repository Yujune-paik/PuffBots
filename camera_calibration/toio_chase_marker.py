...
def adjust_toio_angle(marker_angle):
    # マーカーの角度から90度引いて、0-359度の範囲に収める
    return (marker_angle - 90) % 360
                        ...
                        for i in range(len(ids)):
                            if ids[i][0] not in corner_marker_ids:
                                ...
                                if ids[i][0] == target_marker_id:
                                    target_position = (int(avg_position[0]), int(avg_position[1]))
                                    ...
                                    if position_changed or angle_changed:
                                        print(f"Marker position or angle changed to {target_position}, {target_angle}°")
                                        last_position = target_position
                                        last_angle = target_angle
                                        last_move_time = current_time + 3  # 3秒後に移動
                                    
                                    if current_time >= last_move_time:
                                        print(f"Moving toio to marker {target_marker_id} at position {target_position}, angle {target_angle}°")

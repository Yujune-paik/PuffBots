...
async def move_toio_sequence(cube, x1, y1, angle, x2, y2):
    ...
    center = (250, 250)
    target_x = center[0] + mat_handler.mat_width
    target_y = center[1] + mat_handler.mat_height
    print(f"Moving toio to final position: ({target_x}, {target_y})")
    await mat_handler.move_to_position(target_x, target_y)
    await asyncio.sleep(1)
    
    await mat_handler.unregister_notification_handler()

async def main(target_marker_id=5):
    init_com()
                        ...
                        for i in range(len(ids)):
                            if ids[i][0] not in corner_marker_ids:
                                ...
                                if ids[i][0] == target_marker_id:
                                    toio_x1, toio_y1 = calculate_toio_position(avg_position[0], avg_position[1], marker_angle, distance=10)
